...
use App\Http\Requests\Documents\UpdateDocumentRequirementRequest;
...
class DocumentRequirementController extends Controller
{
    ...
    public function update(UpdateDocumentRequirementRequest $request, DocumentRequirement $documentRequirement): DocumentRequirementResource
    {
        abort_unless($documentRequirement->organization_id === $request->user()->organization_id, 404);

        $documentRequirement->update($request->validated());

        return new DocumentRequirementResource($documentRequirement->refresh()->load(['documentType', 'department', 'designation', 'gradeLevel', 'employmentType', 'location']));
    }
}
